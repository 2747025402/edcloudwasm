...
const manualPipe = async (readable, writable) => {
    ...
    let offset = 0, totalBytes = 0, timerId = null, resume = null, dynamicLowerBound = 4096, lastStatTime = Date.now();
    let currentCounts = [0, 0, 0, 0, 0, 0, 0];
    const lowerBounds = [0, 512, 1024, 2048, 4096, 8192, 24576];
    const flushBuffer = () => {
    ...
    try {
        while (true) {
            ...
            if (now - lastStatTime >= 1000) {
                let maxIdx = 0;
                for (let i = 1; i < 7; i++) {
                    if (currentCounts[i] > currentCounts[maxIdx]) maxIdx = i;
                    if (currentCounts[maxIdx] > 0) dynamicLowerBound = lowerBounds[maxIdx];
                }
                currentCounts = [0, 0, 0, 0, 0, 0, 0];
                lastStatTime = now;
            }
            let idx = 6;
            if (chunkLen < 512) idx = 0;
            else if (chunkLen < 1024) idx = 1;
            else if (chunkLen < 2048) idx = 2;
            else if (chunkLen < 4096) idx = 3;
            else if (chunkLen < 8192) idx = 4;
            else if (chunkLen < 24576) idx = 5;
            currentCounts[idx]++;
            if (chunkLen < 512) {
                flushBuffer(), writable.send(value.slice());
            } else {
